defmodule BotArmyGtd.DecompositionStore do
  ...
  """
  @impl true
  def get(tenant_id, decomposition_id)
      when is_binary(tenant_id) and is_binary(decomposition_id) do
  ...
  """
  @impl true
  def get_by_parent_task(tenant_id, parent_task_id)
      when is_binary(tenant_id) and is_binary(parent_task_id) do
  ...
  def init(_opts) do
    ...
    state =
      try do
        ...
        _ ->
          Logger.warning(
            "Could not load decompositions from database (database unavailable). Starting with empty state."
          )
  ...
  def handle_call({:get_by_parent_task, tenant_id, parent_task_id}, _from, state) do
    result =
      Enum.find(state, fn {_k, decomposition} ->
                 ...
                 changeset =
                   BotArmyGtd.Schemas.Decomposition.changeset(
                     ...
                     %{
                       ...
                       "difficulty" =>
                         Map.get(payload, "difficulty", db_decomposition.difficulty),
                       "due_at" =>
                         parse_due_at(Map.get(payload, "due_at", db_decomposition.due_at)),
                       "review_count" =>
                         Map.get(payload, "review_count", db_decomposition.review_count),
                       "last_grade" =>
                         Map.get(payload, "last_grade", db_decomposition.last_grade),
                       ...
                       "user_rating" =>
                         Map.get(payload, "user_rating", db_decomposition.user_rating),
                       "user_feedback" =>
  ...
  defp schema_to_map(%BotArmyGtd.Schemas.Decomposition{} = decomposition) do
    %{
      ...
      "due_at" =>
        if(decomposition.due_at, do: decomposition.due_at |> DateTime.to_iso8601(), else: nil),
