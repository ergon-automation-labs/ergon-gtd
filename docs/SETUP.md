# Getting Started with bot_army_gtd

This guide walks you through setting up the GTD Bot for local development.
...
This creates a release named `gtd_bot` that is deployed to `/opt/ergon/releases/gtd_bot/` on the server.
...
DATABASE_NAME=bot_army_gtd_dev
```
...
3. Extracts and deploys to `/opt/ergon/releases/gtd_bot/`
